...
#pragma omp parallel for
  ...
  for(std::string beta : betas){
    ...
    std::string path_ens = basedir+get_configname(beta, mass, type)+"_absP.h5";
    ...
    for(int conf=conf_min0; conf<conf_max0; conf++){
      ...
      // std::string pathO3 = basedir2c+ens.id+"absP"+std::to_string(conf)+".xml";
...
#pragma omp critical
      {
        ...
        ens.Os.push_back( abs( all_polyakov.back() ) );
      }
      // {
      //   XmlReader WR( pathO3 );
...
#pragma omp critical
...

  {
    ...
    for(int j=0; j<nbeta; j++){
      ...
      std::string path_ens = basedir+id_str+"_absP.h5";
...
#pragma omp parallel for collapse(2)
    ...
    for(int jdrop=0; jdrop<nbeta; jdrop++){
      for(int ibin=0; ibin<nbin; ibin++){
        ...
        for(int i=0; i<nbeta; i++) ensembles.push_back( ensembles0[i] ); // by copy
        ...
        std::string path_res = basedir+id_str+"meas_absP.h5";
        ...
        FreeEnergies fs(nbeta);
        {
          ...
          // id_str2 = id_str2+"_jk_"+std::to_string(jdrop)+"_"+std::to_string(nbin)+"_"+std::to_string(ibin);
#pragma omp critical
          {
            std::unique_ptr<Hdf5Reader> WR;
